...
export class RokService {
  ...
  public getCodeserverLab(url: string, token: string): Observable<CodeserverLab> {
    // Get the Token needed for Rok Api requests
    ...
    return this.http
      ...
      .pipe(
        ...
        map(resp => {
          ...
          const vscode: CodeserverLab = emptyCodeserverLab();

          // Fill the vscode with the info from the response
          vscode.image = this.getHeader(headers, "X-Object-Meta-image");
          vscode.cpu = this.getHeader(headers, "X-Object-Meta-cpu");
          vscode.memory = this.getHeader(headers, "X-Object-Meta-memory");
          vscode.extra = this.getHeader(
            headers,
