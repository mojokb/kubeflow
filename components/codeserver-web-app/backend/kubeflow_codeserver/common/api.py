...
@auth.needs_authorization("list", "kubeflow.org", "v1beta1", "vscodes")
def list_vscodes(namespace):
    return wrap_resp(
        "vscodes",
...
# We don't do a subject access review on vscode events because
# vscode events are cluster scoped resources. Users however are only
# granted access to particular namespacs. We rely on the vscode webserver
# to filter out information a user shouldn't see.
def list_vscode_events(namespace, nb_name):
    '''
    V1EventList with events whose source the Vscode with 'nb_name' from namespace 'namespace'
    '''
    return wrap_resp(
        "vscode-events",
        v1_core.list_namespaced_event,
        namespace=namespace,
        field_selector="involvedObject.kind=Vscode,involvedObject.name=" + nb_name
    )
...
# POSTers
@auth.needs_authorization("create", "kubeflow.org", "v1beta1", "vscodes")
def create_vscode(vscode, namespace):
    return wrap(
...
# DELETErs
@auth.needs_authorization("delete", "kubeflow.org", "v1beta1", "vscodes")
def delete_vscode(vscode_name, namespace):
    return wrap(
