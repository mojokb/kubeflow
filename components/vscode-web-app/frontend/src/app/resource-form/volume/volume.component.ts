...
export class VolumeComponent implements OnInit, OnDestroy {
  ...
  @Input()
  get vscodeName() {
    return this._vscodeName;
  }
  set vscodeName(nm: string) {
    if (!this.volume.disabled) {
      this.vscodeNameChanged(nm);
    }
  }
  ...
  renderVolName(name: string): string {
    return name.replace('{vscode-name}', this.vscodeName);
  }
  ...
  vscodeNameChanged(nm: string): void {
    if (this.volume.disabled) {
      return;
    }
