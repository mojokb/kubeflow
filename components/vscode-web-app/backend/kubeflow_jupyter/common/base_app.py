...
# REST Routes
@app.route("/api/namespaces/<namespace>/vscodes")
def get_vscodes(namespace):
    data = api.list_vscodes(namespace=namespace)

    if not data["success"]:
        return jsonify(data)

    items = []
    for nb in data["vscodes"]["items"]:
        nb_name = nb["metadata"]["name"]
        nb_creation_time = dt.datetime.strptime(
            nb["metadata"]["creationTimestamp"], "%Y-%m-%dT%H:%M:%SZ")
        nb_events = api.list_vscode_events(namespace, nb_name)
...
# DELETErs
@app.route("/api/namespaces/<namespace>/vscodes/<vscode>",
           methods=["DELETE"])
def delete_vscode(namespace, vscode):
    return jsonify(api.delete_vscode(vscode, namespace=namespace))
